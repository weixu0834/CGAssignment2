...
void main() {
	...
	dimFact = (dimFact + 0.25) * matcol.a + (1.0 - matcol.a);
	vec3 col = (matcol*(1.0-matcol.a) + texture(u_texture, fs_uv)*matcol.a).rgb;
	color = vec4(min(col * dimFact  * (1.0-normvis) * (1.0-UVvis) +
					normvis * (fs_norm + vec3(1.0,1.0,1.0)) / 2.0 + 
					UVvis * vec3(fs_uv, 0.5), vec3(1.0, 1.0, 1.0)), 1.0);
}`;
...
var keyFunction =function(e) {
	if (e.keyCode == 32) {	// Space
		wireframeMode = (wireframeMode + 1) % 4;
	}	
}
...
function drawScene() {
	...
	output.innerHTML = (slider+1) + " Vertices: " + (startVertex[slider+1]-startVertex[slider])+
						" Triangles: " + ((startIndex[slider+1]-startIndex[slider])/3);
	...
	if(wireframeMode == 1) {
		...
		gl.uniform1f(program.normvis, 1.0);
		gl.uniform1f(program.UVvis, 0.0);
