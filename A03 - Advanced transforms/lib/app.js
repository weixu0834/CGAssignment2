...
function InitTransforms() {
	...
	sTransformText[0] = "Rotate 60 degrees around an arbitrary axis passing through (0,1,-1). The x-axis can be aligned to the arbitrary axis after a rotation of 45 degrees around the z-axis, and then 15 degrees around the y-axis";
	...
	sTransformText[1] = "Half the size of the object along a line that bisects the positive x and y axes on the xy-plane";
	sTransform[1] = utils.multiplyMatrices(utils.multiplyMatrices(
		utils.MakeRotateZMatrix(45),
		utils.MakeScaleNuMatrix(0.5, 1, 1)),
		utils.MakeRotateZMatrix(-45));
	
	pTransform[2] =	utils.identityMatrix();
	sTransformText[2] = "Mirror the starship along a plane passing through (1,1,1), and obtained rotating 15 degree around the x axis the xz plane";
	...
	pTransform[3] =	utils.multiplyMatrices(utils.multiplyMatrices(	
		utils.MakeScaleMatrix(3),
		utils.MakeTranslateMatrix(0,0,5)),
		utils.MakeRotateYMatrix(30));
	sTransformText[3] = "Apply the inverse of the following sequence of transforms: rotation of 30 degree around the Y axis then Translation of (0, 0, 5), and finally a uniform scaling of a factor of 3";
