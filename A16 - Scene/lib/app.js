...
var keyFunction =function(e) {
	...
	if (e.keyCode > 48 && e.keyCode < 54) {
		var f = e.keyCode - 48;
		fingers[f] = 1 - fingers[f];
		TheSceneP[f   ][3] = 90 * fingers[f];
		TheSceneP[f+ 5][3] = 90 * fingers[f];
		if(f < 5) {
			TheSceneP[f+10][3] = 90 * fingers[f];
		}
		console.log(TheSceneP);
	}
}
...
TheScene = [
	  [   0,-2.0,0,   0,0, 0,  1, 5,     1,  1,0.2],
	  [-0.9, 2.1,0,   0,0, 0,  6, 6,   0.2, 0.5,0.16], 
	  [-0.3, 2.1,0,   0,0, 0,  7, 7,   0.2, 0.5,0.16], 
	  [ 0.3, 2.1,0,   0,0, 0,  8, 8,   0.2, 0.5,0.16], 
	  [ 0.9, 2.1,0,   0,0, 0,  9, 9,   0.2, 0.5,0.16], 
	  [-1.1, 0.2,0,   0,0,90, 10,10,   0.2, 0.5,0.16],
	  [ 0.0, 1.1,0,   0,0, 0, 11,11,   0.2,0.35,0.16], 
	  [ 0.0, 1.1,0,   0,0, 0, 12,12,   0.2,0.35,0.16], 
	  [ 0.0, 1.1,0,   0,0, 0, 13,13,   0.2,0.35,0.16], 
	  [ 0.0, 1.1,0,   0,0, 0, 14,14,   0.2,0.35,0.16], 
	  [ 0.0, 1.1,0,   0,0, 0, -1,-1,   0.2,0.35,0.16], 
	  [ 0.0, 0.8,0,   0,0, 0, -1,-1,   0.2,0.25,0.16], 
	  [ 0.0, 0.8,0,   0,0, 0, -1,-1,   0.2,0.25,0.16], 
	  [ 0.0, 0.8,0,   0,0, 0, -1,-1,   0.2,0.25,0.16], 
	  [ 0.0, 0.8,0,   0,0, 0, -1,-1,   0.2,0.25,0.16], 
	];
...
for(i = 0; i < TheScene.length; i++) {
	TheSceneP[i] = [];
	for(j = 0; j < 8; j++) {TheSceneP[i][j] = TheScene[i][j];}
}
