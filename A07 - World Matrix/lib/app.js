...
function InitTransforms() {
	...
	tTransform = vi.concat(world());

	sTransformText[0] = "Positioned in 0,0,-3. Yaw=90, Pitch and Roll = 0";
	sTransform[0] = utils.MakeWorld(0,0,-3, 0,90,0, 1);
		
	sTransformText[1] = "Positioned in 0,2,0. Yaw=0, Pitch = 60, Roll = 0, 1/10th of size";
	sTransform[1] = utils.MakeWorld(0,2,0, 60,0,0, 1/10);
	
	sTransformText[2] = "Positioned in 0,0,0. Yaw=30, Pitch = 0 Roll = 45";
	sTransform[2] = utils.MakeWorld(0,0,0, 0,30,45, 1);

	
	sTransformText[3] = "Positioned in 2,0,2. Yaw=180, Pitch and Roll = 0, two times wider";
	sTransform[3] = utils.MakeWorldNu(2,0,2, 0,180,0, 2,1,1);
	       	       
	sTransformText[4] = "Positioned in 1,-1,2.5. Yaw=-30, Pitch = 45 Roll = -15, Scaled with the following factors: 0.8 (x), 0.75 (y), 1.2 (z)";
	sTransform[4] = utils.MakeWorldNu(1,-1,2.5, 45,-30,-15, 0.8,0.75,1.2);
...
function drawScene() {
		...
		preRot = utils.MakeRotateYMatrix(-90);

		// sets the uniforms
		gl.uniform1i(program.textureUniform, 0);

		// draws the request
		WVPmatrix = utils.multiplyMatrices(utils.multiplyMatrices(prjMatrix, viewMatrix), utils.multiplyMatrices(sTransform[curr_sTransform], preRot));
		gl.uniformMatrix4fv(program.WVPmatrixUniform, gl.FALSE, utils.transposeMatrix(WVPmatrix));		
		gl.drawElements(gl.LINE_STRIP, mesh.indexBuffer.numItems, gl.UNSIGNED_SHORT, 0);		
		
		// draws the answer
		if(!changeT) {
			WVPmatrix = utils.multiplyMatrices(utils.multiplyMatrices(prjMatrix, viewMatrix), utils.multiplyMatrices(tTransform[curr_tTransform], preRot));
			gl.uniformMatrix4fv(program.WVPmatrixUniform, gl.FALSE, utils.transposeMatrix(WVPmatrix));		
